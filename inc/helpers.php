 ...
 */
function gatewayapi__extract_tags_from_message($message)
{
  $simple_search_for = array_keys(gatewayapi__all_tags());
...
function gatewayapi__get_recipient($cc, $number)
{
  $number = preg_replace('/\D+/', '', sanitize_key($number));
  $cc = preg_replace('/\D+/', '', sanitize_key($cc));
...
function gatewayapi__get_tag_specification($tag)
{
  static $tag_defs;
 ...
 * @return string
 */
function gatewayapi__receive_sms_token()
{
...
function gatewayapi__receive_sms_url()
{
  if (get_option('gwapi_receive_sms_enable')) {
    return admin_url('admin-ajax.php?action=gatewayapi_receive_sms&token=' . gatewayapi__receive_sms_token());
  }
}
...
{
  $phone = preg_replace('/\D+/', '', sanitize_key($cc) . ltrim(sanitize_key($number), '0'));
  return $phone;
}
        ...
        <script>
          jQuery(document).ready(function ($) {
            $("select#post_status").append(
              ...
              '<?php echo $complete ? 'selected="selected"' : ''; ?>>' +
              '<?php echo esc_html(_BitCptStatus::$status_names[$status]); ?></option>'
            );

            <?php if ($complete): ?>
            $("#post-status-display").text(<?php echo json_encode(_BitCptStatus::$status_names[$status]); ?>);
            <?php endif; ?>
          });
        </script>
        <?php
  ...
  add_action('admin_footer-edit.php', function () {
    ...
    if ($post && in_array($post->post_type, array_keys(_BitCptStatus::$cpt_status))) {
      foreach (_BitCptStatus::$cpt_status[$post->post_type] as $status) {
        ...
        <script>
          jQuery(document).ready(function ($) {
            $('select[name="_status"]').append('<option value="<?php echo esc_attr($status); ?>">' +
              <?php echo json_encode(_BitCptStatus::$status_names[$status]);?>+
                '</option>'
            );
          });
        </script>
 ...
 * @return null
 */
function gatewayapi__render_template($template, ...$args)
{
  ...
  $template_file = gatewayapi__dir() . '/tpl/' . $template . '.php';

  if (file_exists($template_file)) {
    include gatewayapi__dir() . '/tpl/' . $template . '.php';
  }
