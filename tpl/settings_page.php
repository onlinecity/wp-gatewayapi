    ...
    <div class="tab-inner">
      ...
      <div data-tab="base" class="tab hidden" id="baseTab">
        ...
        <p>
          ...
          <?php echo strtr(__('Please enter your OAuth Key and OAuth Secret below. You find this information by logging into :link and then navigate to <strong>Settings » API Keys</strong>.', 'gatewayapi'), $link); ?>
        </p>
        ...
        <table class="form-table">
          <tr valign="top">
            ...
            <td>
              <input type="text" name="gwapi_key" value="<?php echo esc_attr(get_option('gwapi_key')); ?>" size="32"/>
            </td>
          </tr>
          ...
          <tr valign="top">
            ...
            <td>
              <label>
                <input type="checkbox" name="gwapi_enable_ui" <?php echo get_option('gwapi_enable_ui') ? 'checked' : ''; ?>>
          ...
          <tr valign="top">
            ...
            <td>
              <label>
                <input type="checkbox"
                       name="gwapi_security_enable" <?php echo get_option('gwapi_security_enable') ? 'checked' : ''; ?>
            ...
            <td>
              <label>
                <input type="text" maxlength="15" name="gwapi_default_sender"
                       value="<?php echo esc_attr(get_option('gwapi_default_sender')); ?>">
              </label>
              <p class="help-block description">
        ...
        <div id="enableCaptcha" <?php echo !get_option('gwapi_enable_ui') ? 'class="hidden"' : ''; ?>>
          ...
          <p><?php echo strtr(__('<a href="%url%" target="_blank"><strong>Click here</strong></a> to read more about reCAPTCHA and signup.', 'gatewayapi'), ['%url%' => 'https://www.google.com/recaptcha']); ?></p>
              ...
              <td>
                <label>
                  <input type="text" size="50"
                         ...
                         value="<?php echo esc_attr(get_option('gwapi_recaptcha_site_key')); ?>">
                </label>
              </td>
            </tr>
            <tr valign="top">
              ...
              <td>
                <label>
                  <input type="text" size="50"
                         ...
                         value="<?php echo esc_attr(get_option('gwapi_recaptcha_secret_key')); ?>">
                </label>
              </td>
            </tr>
          </table>
      ...
      <div data-tab="security" class="tab hidden" id="securityTab">
        <p>
          <?php _e('The two-factor login system is based solely on SMS\'es, so your users will not need any apps, thus making it compatible with any mobile phone. All you will ever pay, is the cost of the text messages sent as part of the login process, while getting the greatly added security of two-factor security.', 'gatewayapi'); ?>
        </p>
        ...
        <table class="form-table">
          <?php if (get_option('gwapi_security_enable')): ?>
            <tr valign="top">
              ...
              ?>
              <td>
                <input type="hidden" name="gwapi_security_bypass_code" value="<?php echo esc_attr(GwapiSecurityTwoFactor::getBypassCode()); ?>"/>
                <input type="text" size="85" readonly value="<?php echo esc_attr($login_bypass_url); ?>"
                       placeholder="<?php _e('New code is generated on save', 'gatewayapi'); ?>"/>
                <button id="gwapiSecurityBypassCodeReset" type="button" class="button button-secondary"><?php _e('Reset', 'gatewayapi'); ?></button>
                <p class="help-block description">
                  <strong
                    style="color: blue"><?php _e('This URL should be copied to a safe place!', 'gatewayapi'); ?></strong> <?php _e('Use it to bypass all two-factor security measures when logging in.', 'gatewayapi'); ?>
                  <i class="info has-tooltip"
                     title="<?php esc_attr_e('This could rescue you from a site lockout, in case your GatewayAPI-account ran out of credit (you should enable auto-charge to avoid this) or if you forgot to update your profile when you got a new number. You should not share this URL, but keep it as a recovery measure for you as an administrator.', 'gatewayapi'); ?>"></i>
                </p>
              </td>
            </tr>
            <tr valign="top">
              ...
              <td>
                ...
                <select name="gwapi_security_required_roles[]" multiple
                        size="<?php echo esc_attr(min(count($roles), 6)); ?>"
                        style="min-width: 250px;">
                  <?php foreach ($roles as $role_key => $role_opt): ?>
                    <option value="<?php echo esc_attr($role_key); ?>" <?php echo $role_opt[1] ? 'selected' : '' ?>>
                      <?php echo esc_html($role_opt[0]); ?></option>
                  <?php endforeach; ?>
                </select>
                <p class="help-block description">
              ...
              <td>
                <select name="gwapi_security_cookie_lifetime">
                  ...
                  <?php foreach ($options as $days => $text): ?>
                    <option <?php echo get_option('gwapi_security_cookie_lifetime') == $days ? 'selected' : ''; ?>
                      value="<?php echo esc_attr($days); ?>"><?php echo esc_html($text); ?></option>
                  <?php endforeach; ?>
                </select>
                <p class="help-block description">
      ...
      <div class="tab hidden" data-tab="recipients-fields" id="recipientsTab">
        <?php
        $all_fields = gatewayapi__all_recipient_fields();
        ...
        <div class="recipient-fields">
          <?php foreach ($all_fields as $af): ?>
            <div class="recipient-field <?php echo $af['type'] == 'template' ? 'hidden' : ''; ?>"
              <?php echo $af['type'] == 'template' ? 'data-is-template' : '' ?>
              <?php echo isset($af['is_builtin']) && $af['is_builtin'] ? 'data-is-builtin' : '' ?>>
              <?php echo isset($af['is_builtin']) && $af['is_builtin'] ? '<input type="hidden" name="gwapi_recipient_fields[is_builtin][]" value="1">' : ''; ?>
              ...
              <div class="field-group label-left with-drag with-decoration">
                ...
                <div class="form-field">
                  <label class="control-label">
                    ...
                    <i class="info has-tooltip"
                       title="<?php esc_attr_e('Shown in all pulic forms which uses this field.', 'gatewayapi'); ?>"></i>
                  </label>
                  <div class="form-control">
                    <input required type="text" name="gwapi_recipient_fields[name][]"
                           value="<?php echo esc_attr(isset($af['name']) ? $af['name'] : ''); ?>">
                  </div>
                </div>
                ...
                <div class="form-field">
                  <label class="control-label">
                    ...
                    <i class="info has-tooltip"
                       title="<?php esc_attr_e('This is used as the tag when sending SMS\'es, in shortcodes and is used as the meta key.', 'gatewayapi'); ?>"></i>
                  </label>
                  <div class="form-control">
                    <input
                      required <?php echo isset($af['is_builtin']) && $af['is_builtin'] ? 'disabled' : ''; ?>
                      type="text" name="gwapi_recipient_fields[field_id][]"
                      value="<?php echo esc_attr(isset($af['field_id']) ? $af['field_id'] : ''); ?>">
                ...
                <div class="form-field">
                  <label class="control-label">
                    ...
                    <i class="info has-tooltip"
                       title="<?php esc_attr_e('A helpful description of what input the user is expected to put in the field.', 'gatewayapi'); ?>"></i>
                  </label>
                  <div class="form-control">
                    <input type="text" name="gwapi_recipient_fields[description][]"
                           value="<?php echo esc_attr(isset($af['description']) ? $af['description'] : ''); ?>">
                  </div>
                </div>
                ...
                <div class="form-field" data-hidden_on="hidden,radio">
                  <label class="control-label">
                    ...
                    <i class="info has-tooltip"
                       title="<?php esc_attr_e('Required fields must be filled when adding recipients. For multiple choice field types, at least one value must be picked.', 'gatewayapi'); ?>"></i>
                  </label>
                  <div class="form-control">
                    <label>
                      <input <?php echo isset($af['is_builtin']) && $af['is_builtin'] ? 'disabled' : ''; ?>
                        type="checkbox" name="gwapi_recipient_fields[required][]"
                        value="1" <?php echo (isset($af['required']) && $af['required']) ? 'checked' : ''; ?>>
                      <?php _e('Yes, this is a required field.'); ?>

                      <?php if (isset($af['is_builtin']) && $af['is_builtin']): ?>
                        <input type="hidden" name="gwapi_recipient_fields[required][]"
                               value="<?php echo esc_attr(isset($af['required']) ? $af['required'] : ''); ?>">
                      <?php endif; ?>
                    </label>
                  </div>
                </div>
                ...
                <div class="form-field">
                  ...
                  <?php
                  $all_types = gatewayapi__all_recipient_field_types(true);
                  ?>
                  <div class="form-control">
                    <select <?php echo isset($af['is_builtin']) && $af['is_builtin'] ? 'disabled' : ''; ?>
                        ...
                        <optgroup label="<?php echo esc_attr($optgroup); ?>">
                          <?php foreach ($fields as $key => $val): ?>
                            <option value="<?php echo esc_attr($key); ?>"
                              <?php echo (isset($af['type']) && $af['type'] == $key) ? 'selected' : '' ?>>
                    ...
                    <?php if (isset($af['is_builtin']) && $af['is_builtin']): ?>
                      <input type="hidden" name="gwapi_recipient_fields[type][]"
                             value="<?php echo esc_attr($af['type'] ?? ''); ?>">
                    <?php endif; ?>
                  </div>
                </div>
                ...
                <div class="form-field hidden" data-visible_on="mobile_cc">
                  <label class="control-label">
                    ...
                    <i class="info has-tooltip"
                       title="<?php esc_attr_e('If you wish to limit the countries available, please enter the country calling codes below (one per line, no “+”-prefix).', 'gatewayapi'); ?>"></i>
                    <br/>
                ...
                <div class="form-field hidden" data-visible_on="select,radio,checkbox">
                  <label class="control-label">
                    ...
                    <i class="info has-tooltip"
                       title="<?php esc_attr_e('Enter a single choice per line. Prefix the line with two exclamation marks (!!), to make an option selected by default.', 'gatewayapi'); ?>"></i>
                  </label>
                  <div class="form-control">
                    <textarea name="gwapi_recipient_fields[choices][]"
                              rows="5"><?php echo esc_attr($af['choices'] ?? ''); ?></textarea>
                  </div>
                </div>
                  ...
                  <div class="form-field">
                    <div class="form-control">
                      <button type="button" class="button button-danger" data-delete="true"
                              data-warning="<?php esc_attr_e('Are you sure that you want to delete this field?', 'gatewayapi'); ?>">
                        <?php _e('Delete this field', 'gatewayapi'); ?>
                      </button>
                    </div>
                  </div>
                <?php endif; ?>
        ...
        <button class="button" type="button" data-reset-btn
                data-warning="<?php esc_attr_e('Are you sure that you want to reset to default? All your field configurations above will be lost immediately with no way of recovery.', 'gatewayapi'); ?>">
          <?php _e('Reset to default', 'gatewayapi'); ?>
        </button>
        <br><br>
      </div>
      <!-- RECIPIENT FIELDS -->
        ...
        <p>
          <label>
            <input type="checkbox" name="gwapi_user_sync_enable" value="1"
                   id="userSyncEnableCb" <?php echo get_option('gwapi_user_sync_enable') ? 'checked' : ''; ?>> <?php _e('Enable user synchronization',
              'gatewayapi'); ?>
          </label>
        </p>
        <div id="userSyncEnabled" <?php echo get_option('gwapi_user_sync_enable') ? '' : 'class="hidden"'; ?>>
          <table class="form-table">
            <tr valign="top">
              ...
              <td>
                <div style="max-width: 400px">
                  <select name="gwapi_user_sync_meta_default_countrycode"
                          data-default-cc="<?php echo esc_attr(get_option('gwapi_user_sync_meta_default_countrycode')); ?>">
                  </select>
                </div>
                <p class="description">
            ...
            <tr valign="top">
              ...
              <td>
                <textarea name="gwapi_user_sync_meta_other_fields" id="" cols="50" rows="6"
                          placeholder="<?php esc_attr_e('Meta Key : Recipient Field Key', 'gatewayapi'); ?>"
                ><?php echo esc_html(get_option('gwapi_user_sync_meta_other_fields')); ?></textarea>
                <p class="description">
                    ...
                    <tr>
                      ...
                      <td style="padding: 5px 10px 0 0">
                        <input type="text"
                               ...
                               value="<?php echo esc_attr($gmap[$g->term_id] ?? null); ?>">
                      </td>
                    </tr>
                    <?php
                  }
        ...
        <p>
          <label>
            <input type="checkbox" name="gwapi_receive_sms_enable" value="1"
                   id="receiveSmsEnableCb" <?php echo get_option('gwapi_receive_sms_enable') ? 'checked' : ''; ?>> <?php _e('Yes please, I would like to receive SMS messages.', 'gatewayapi'); ?>
          </label>
        </p>

        <?php if (get_option('gwapi_enable_ui') && get_option('gwapi_receive_sms_enable')): ?>
          <div id="receiveSmsEnabled" <?php echo get_option('gwapi_receive_sms_enable') ? '' : 'class="hidden"'; ?>>
                  ...
                  <li>
                    ...
                    <div class="description" style="text-decoration:underline">
                      <input name="gwapi_receive_sms_url" style="width:100%"
                             value="<?php echo esc_attr(gatewayapi__receive_sms_url()); ?>">
                    </div>
                  </li>
                </ol>
              </tr>
